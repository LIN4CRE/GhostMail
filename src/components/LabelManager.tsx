...
export function LabelManager() {
  const { showLabelManager, setShowLabelManager, labels, createLabel, deleteLabel } = useStore();
  ...
  if (!showLabelManager) return null;

  const handleCreate = () => {
    if (!newName.trim()) return;
    createLabel(newName.trim(), selectedColor);
    setNewName('');
  };
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        ...
        <motion.div initial={{ opacity: 0, scale: 0.92, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
          ...
          <div className="flex items-center gap-3 border-b border-tn-border px-5 py-3.5">
            ...
            <button onClick={() => setShowLabelManager(false)} className="rounded-lg p-1.5 text-tn-fg-muted hover:bg-tn-bg-highlight hover:text-tn-fg"><X size={16} /></button>
          </div>

          <div className="p-5 space-y-4 max-h-[60vh] overflow-y-auto">
            {/* Create */}
            <div className="rounded-xl border border-tn-border bg-tn-bg-highlight/50 p-4">
              <h3 className="text-xs font-semibold text-tn-fg mb-2.5 flex items-center gap-1.5"><Plus size={12} /> Create Label</h3>
              <div className="flex gap-2 mb-2.5">
                ...
                <motion.button onClick={handleCreate} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                  className="rounded-lg bg-tn-blue/20 px-4 py-2 text-sm font-medium text-tn-blue hover:bg-tn-blue/30">Create</motion.button>
              </div>
                  ...
                  {userLabels.map(l => (
                    <div key={l.id} className="flex items-center gap-2.5 rounded-xl bg-tn-bg-highlight/30 px-3 py-2.5 hover:bg-tn-bg-highlight transition-colors group">
                      ...
                      <button onClick={() => deleteLabel(l.id)}
                        className="rounded p-1 text-tn-fg-muted opacity-0 group-hover:opacity-100 hover:text-tn-red transition-all">
                        <Trash2 size={12} />
                      </button>
                    </div>
                  ))}
