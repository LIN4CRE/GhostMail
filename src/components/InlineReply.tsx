  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 border-b border-tn-border/50 px-4 py-2.5">
        <span className="text-[11px] text-tn-fg-muted font-medium">Reply to</span>
        <span className="rounded-full bg-tn-bg-highlight px-2.5 py-0.5 text-[11px] text-tn-fg-dark">
          {recipientName}
        </span>
        <div className="flex-1" />
        <span className="text-[9px] text-tn-fg-muted">&lt;{recipientEmail}&gt;</span>
        <button onClick={onClose} className="rounded-md p-1 text-tn-fg-muted hover:text-tn-fg"><X size={13} /></button>
      </div>

      <textarea value={body} onChange={e => setBody(e.target.value)} placeholder="Write your reply..."
        rows={4} autoFocus
