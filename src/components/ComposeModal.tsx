...
export function ComposeModal() {
  const { showCompose, setShowCompose, notify } = useStore();
  ...
  const reset = () => { setTo(''); setCc(''); setBcc(''); setSubject(''); setBody(''); setShowCcBcc(false); };
      ...
      {showCompose && (
        <>
          ...
          <motion.div
            ...
            animate={minimized ? { opacity: 1, y: 0, scale: 1 } : { opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: 40, scale: 0.95 }}
            transition={{ type: 'spring', stiffness: 300, damping: 30 }}
            className={`fixed z-50 overflow-hidden rounded-2xl border border-tn-border shadow-2xl ${
              minimized
                ? 'bottom-4 right-4 w-80'
                : 'bottom-4 right-4 w-[580px] max-w-[calc(100vw-24px)] sm:bottom-auto sm:right-auto sm:left-1/2 sm:top-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2'
            }`}
          >
            {/* Header */}
            <div className="flex items-center gap-2 bg-gradient-to-r from-tn-bg-dark via-tn-bg to-tn-bg-dark px-4 py-2.5 border-b border-tn-border/50">
              ...
              {!minimized && <span className="text-[9px] text-tn-fg-muted">{wordCount} words</span>}
            ...
            {!minimized && (
              <div className="bg-tn-bg-darker">
                ...
                <div className="flex items-center border-b border-tn-border/30 px-4 py-2">
                  ...
                  {!showCcBcc && (
                    <button onClick={() => setShowCcBcc(true)} className="text-[10px] text-tn-fg-muted hover:text-tn-blue transition-colors">
                      Cc/Bcc
                    </button>
                  )}
                </div>
                  ...
                  {showCcBcc && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}>
                      ...
                      <div className="flex items-center border-b border-tn-border/30 px-4 py-2">
                        ...
                        <input type="email" value={bcc} onChange={e => setBcc(e.target.value)} placeholder="bcc@example.com"
                          className="flex-1 bg-transparent text-sm text-tn-fg placeholder-tn-fg-muted/40 outline-none" />
                      </div>
                    </motion.div>
                  )}
                ...
                <div className="flex items-center gap-0.5 border-b border-tn-border/30 px-3 py-1">
                  {[Bold, Italic, Link, List, Image].map((Icon, i) => (
                    <button key={i} className="rounded-md p-1.5 text-tn-fg-muted hover:bg-tn-bg-highlight hover:text-tn-fg transition-colors">
                      <Icon size={13} />
                    </button>
                  ))}
                </div>
